...
def _make_tensors(M: int, K: int, N: int,
                  device: str = "cpu",
...
@pytest.mark.parametrize(
    ...
    [
        (16,   64,  128),   # small
        (64,   128, 256),   # medium
        (256,  512, 512),   # single self-attn block QKV projection (L=1, batch×L large)
        (8192, 512, 1536),  # Cortex-S production: B×L=32×256=8192, K=512, N=3×512 (QKV)
        (8192, 512, 2048),  # Cortex-S MLP first layer: K=512, N=4×512
